 ...
 * @date 07/07/2023
 */
class GetDistrictUseCase @Inject constructor(private val registerRepository: RegisterRepository) {

    operator fun invoke(idJson : String) = flow {
        emit(UiEvent.Loading())
        emit(UiEvent.Success(registerRepository.getKecamatan(idJson)))
    }.catch {
