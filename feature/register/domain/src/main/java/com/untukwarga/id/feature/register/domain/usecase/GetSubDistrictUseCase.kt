 ...
 * @date 07/07/2023
 */
class GetSubDistrictUseCase @Inject constructor(private val registerRepository: RegisterRepository) {

    operator fun invoke(idJson : String) = flow {
        emit(UiEvent.Loading())
        emit(UiEvent.Success(registerRepository.getKelurahan(idJson)))
    }.catch {
